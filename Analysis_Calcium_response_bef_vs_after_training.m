...
%        Myj = 1:4-sum(cellfun(@isempty, {A{which_group,2}{J_larv,:}}));
    ...
    % get the names of the tiff files for one indiv:    
       Fol = Dir_Names{J};
       List_filenames = list_files(MyPath,Dir_Names{J},...
           'LexA','fluo','xx','xx');
...
% Load the data ***********************************************************        
            ...
            load([MyPath,Dir_Names{J},'/',filename,'_fluo.mat']);

             % Load uFExp variable (metadata)
            load([MyPath,Dir_Names{J},'/',filename,'_meta.mat']); 
...
save_it_all(MyPath2save,A{which_group,1},Statistics,N_plot,'train',length(cs))
...
rows = cellfun(@(M) size(M,2), vars_CSp);
cols = cellfun(@(M) size(M,1), vars_CSp);
maxrows = max(rows);
